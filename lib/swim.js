...
function Swim(opts) {
  ...
  this.joinCheckInterval =
    opts.joinCheckInterval || Swim.Default.joinCheckInterval;
...
Swim.prototype.join = function join(hosts, callback) {
  ...
  function checkJoin() {
    var numberOfHostsResponded = hosts.reduce(function countRespondedHosts(
      num,
      host
    ) {
...
Swim.prototype.leave = function leave() {
  this.membership.removeListener(
...
Swim.prototype.localhost = function localhost() {
  return (
    (this.membership && this.membership.localhost()) || this.opts.local.host
  );
};
