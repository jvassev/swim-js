...
Net.prototype.sendMessages = function sendMessages(messages, host) {
  var self = this;
  var bytesAvailable =
    self.udp.maxDgramSize - Net.MessageTypeSize - Net.LengthSize;
  var buffers = [];
  var buffer;
  var message;
  var i;
...
Net.prototype.sendMessage = function sendMessage(message, host) {
  ...
  if (message.data) {
    this.piggybackAndSend(
      Buffer.concat([header, this.swim.codec.encode(message.data)]),
      host
    );
  } else {
