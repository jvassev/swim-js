...
function Disseminator(opts) {
  ...
  this.disseminationFactor =
    opts.disseminationFactor || Disseminator.Default.disseminationFactor;
  this.disseminationFormula =
    opts.disseminationFormula || Disseminator.Default.disseminationFormula;
...
Disseminator.prototype.stop = function stop() {
  this.swim.membership.removeListener(
    Membership.EventType.Change,
    this.changeListener
  );
  this.swim.membership.removeListener(
    Membership.EventType.Update,
    this.updateListener
  );
};
...
Disseminator.prototype.updateDisseminationLimit =
  function updateDisseminationLimit() {
    const self = this;

    self.disseminationLimit = self.disseminationFormula(
      self.disseminationFactor,
      self.swim.membership.size(true)
    );

    Object.keys(self.attemptsToUpdates).forEach(function removeAttempts(
      attempts
    ) {
      if (attempts >= self.disseminationLimit) {
        Object.keys(self.attemptsToUpdates[attempts]).forEach(
          function removeUpdate(host) {
...
Disseminator.prototype.removeUpdate = function removeUpdate(update) {
  if (this.hostToAttempts[update.host] >= 0) {
    delete this.attemptsToUpdates[this.hostToAttempts[update.host]][
      update.host
    ];
  }
...
Disseminator.prototype.getUpdatesUpTo = function getUpdatesUpTo(
  bytesAvailable
) {
  const self = this;
  const buffers = [];
...
Disseminator.prototype.serializeUpdate = function serializeUpdate(update) {
  const header = Buffer.alloc(Net.MessageTypeSize);

  Net.WriteMessageType.call(header, MessageType.Update, 0);

  return Buffer.concat([
    header,
    this.swim.codec.encode({
      meta: update.meta,
      host: update.host,
      state: update.state,
