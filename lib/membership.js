...
function Membership(opts) {
  ...
  this.suspectTimeout =
    opts.suspectTimeout || Membership.Default.suspectTimeout;
  this.preferCurrentMeta =
    opts.preferCurrentMeta || Membership.Default.preferCurrentMeta;
...
Membership.prototype.start = function start() {
  ...
  self.swim.failureDetector.on(
    FailureDetector.EventType.Suspect,
    self.suspectListener
  );
...
Membership.prototype.stop = function stop() {
  ...
  self.swim.failureDetector.removeListener(
    FailureDetector.EventType.Suspect,
    self.suspectListener
  );
  ...
  Object.keys(self.hostToSuspectTimeout).forEach(
    function clearTimeoutWithoutDeletion(host) {
...
Membership.prototype.onAck = function onAck(data, host) {
  if (
    this.hostToMember[host] &&
    this.hostToMember[host].state === Member.State.Suspect
  ) {
...
Membership.prototype.onSync = function onSync(data) {
  ...
  this.swim.net.sendMessages(
    this.all(true, true).map(function toMessage(data) {
      return {
        type: MessageType.Update,
        data: data,
      };
...
Membership.prototype.updateAlive = function updateAlive(data) {
  ...
  if (
    this.hostToFaulty[data.host] &&
    this.hostToFaulty[data.host].incarnation >= data.incarnation
  ) {
    this.emit(Membership.EventType.Drop, data);
    return;
  }
  ...
  if (
    ...
    if (!this.hostToMember[data.host]) {
      ...
      this.emit(
        ...
        this.hostToMember[data.host].data()
      );
    } else {
      this.hostToMember[data.host] = new Member(data);
    }
